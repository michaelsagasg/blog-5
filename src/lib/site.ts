...
const BASE = import.meta.env.BASE_URL.replace(/\/$/, ''); // '' at the domain root, '/shotdiaryuk' under a subpath
...
const CAT_BLURB: Record<string, string> = {
  sourcing: 'What to have ready, and where it comes from, before week one.',
  usage: 'Injection sites, weekly logs and what a tracked entry should include.',
  'general-info': 'The basics worth knowing before you start a log.',
};
...
// Bullet-diary tones: warm-olive for sourcing, coral-highlighter for usage (the log entry
// itself), ink-navy for general-info -- no purple, no blue/teal gradients.
const TONES = ['olive', 'coral', 'navy'] as const;
const CAT_TONE: Record<string, (typeof TONES)[number]> = { sourcing: 'olive', usage: 'coral', 'general-info': 'navy' };
export const catTone = (c: string) => CAT_TONE[c] ?? TONES[[...c].reduce((n, ch) => n + ch.charCodeAt(0), 0) % TONES.length];

// A small tally-mark tag stands in for blog-1's element-letter device -- this blog's motif is
// the weekly log entry, not the periodic table.
const CAT_TAG: Record<string, string> = { sourcing: 'Prep', usage: 'Wk', 'general-info': 'Ref' };
